...
namespace SunEvents
{
    class Twilight
    {
        ...
        public void RenderData(int dayOffset, SunAndMoonData dataToday, StringBuilder sb)
        {
            sb.AppendFormat("  Dawn\n    {0}\r\n", FormatDateTime(dataToday.CivalTwilightStart));
            sb.AppendFormat("  Sunrise\n    {0}\r\n", FormatDateTime(dataToday.SunRise));
            sb.AppendFormat("  SunSet\n    {0}\r\n", FormatDateTime(dataToday.SunSet));
            sb.AppendFormat("  Dusk\n    {0}\r\n", FormatDateTime(dataToday.CivalTwilightEnd));
            sb.AppendFormat("  Daylight Hours\n    {0}\r\n", FormatDayLength(dataToday.SunRise, dataToday.SunSet));

            //sb.AppendFormat("  Moon Rise\n    {0}\n\n", FormatDateTime(dataToday.MoonRise));
            //sb.AppendFormat("  Moon Set\n    {0}\n\n", FormatDateTime(dataToday.MoonSet));
            //sb.AppendFormat("  Moon Phase\n    {0}\n\n", FormatMoonData(dayOffset));
        }

        //private string FormatMoonData(int DayOffset)
        //{
        //    Moon m = new Moon();
        //    MoonDetails md = m.GetMoonPosition(DateTime.Now.AddDays(DayOffset));
        //    return m.MoonPhaseToString(md.phase);

        //}

        public string FormatDayLength(DateTime SunRise, DateTime SunSet)
        {
            TimeSpan ts1 = SunSet - SunRise;

            return ts1.Hours + " hours and " + ts1.Minutes.ToString("0") + " minutes";
        }
    }
}
